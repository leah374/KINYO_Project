...
def generate_keyframes(
    ...
    api_key = os.getenv("OPENAI_API_KEY") or os.getenv("K_TOKEN_API_KEY")
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY or K_TOKEN_API_KEY is required when --generate-images is set.")
    ...
    rows = list(iter_prompt_rows(storyboard_payload))
    ...
    for idx, row in enumerate(rows):
        ...
        if progress_callback:
            progress_callback(
                int(current / total_shots * 100),
                f"Generating {shot_id} ({current}/{total_shots})..."
            )
        ...
        for attempt in range(1, retries + 2):
            try:
                ...
                if attempt > retries:
                    raise
                wait_time = min(6 * attempt, 30)
                if progress_callback:
                    progress_callback(
                        int(current / total_shots * 100),
                        f"Retry {attempt}/{retries} for {shot_id}, waiting {wait_time}s..."
                    )
    ...
    if progress_callback:
        progress_callback(100, f"Completed! Generated {len([g for g in generated if g['status'] == 'generated'])} images.")

    return generated


def generate_keyframes_with_products(
    storyboard_payload: Dict[str, Any],
    output_dir: Path,
    base_url: Optional[str],
    image_model: str,
    ...
    tv_screens: List[str] = None,
) -> List[Dict[str, Any]]:
    """
    Generate keyframes using images.edit() to upload product images
    
    Args:
        storyboard_payload: Storyboard data
        output_dir: Output directory
        base_url: API base URL
        image_model: Image model
        size: Image size
        quality: Image quality
        sleep_sec: Sleep between calls
        retries: Number of retries
        overwrite: Overwrite existing images
        skip_shots: Shot IDs to skip
        progress_callback: Progress callback
        product_images: List of product image paths to upload
        tv_screens: List of TV screen descriptions
    ...
    api_key = os.getenv("OPENAI_API_KEY") or os.getenv("K_TOKEN_API_KEY")
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY or K_TOKEN_API_KEY is required.")
    ...
    for idx, row in enumerate(rows):
        ...
        # Select product image (rotate through available products)
        product_path = None
        if product_images:
            product_path = Path(product_images[idx % len(product_images)])
        
        # Build prompt for product integration
        tv_desc = ""
        if tv_screens:
            tv_desc = Path(tv_screens[idx % len(tv_screens)]).stem if tv_screens else ""
        
        prompt = f"""
Create a product showcase keyframe {shot_id}.

IMPORTANT: NO HUMANS, NO FACES, NO PEOPLE, NO HANDS.

Transform this product image into a professional product photography scene:
- Place the product in a modern Chinese living room setting
- TV screen in background displaying: {tv_desc if tv_desc else "karaoke interface"}
- Scene: {row.get('prompt', '')[:200]}
- Warm commercial lighting
- Clean, professional composition

What to include:
- Modern TV wall with displayed interface
- Clean living room environment
...
Style: Commercial product photography, clean, professional.
"""
        
        response = None
        
        # Use images.edit() to upload product image
        if product_path and product_path.exists():
            ...
            # Fallback to generate() if no product image
            for attempt in range(1, retries + 2):
                try:
                    response = client.images.generate(
                        model=image_model,
        ...
        image_b64 = extract_image_b64(response)
        if not image_b64:
            raise RuntimeError(f"Image response for {shot_id} did not include base64 data.")
        ...
        generated.append({
            ...
            "size": size,
            "quality": quality,
            "product_used": str(product_path) if product_path else "",
            "tv_screen": tv_desc,
            "status": "generated"
        })
...
def run(
    ...
    """
    Generate storyboard and keyframes
    
    Args:
        input_path: Path to script JSON
        output_dir: Output directory
        ...
        retries: Number of retries
        overwrite: Overwrite existing images
        skip_shots: List of shot IDs to skip
        progress_callback: Progress callback
        product_images: List of product image paths (uploaded to API)
        tv_screens: List of TV screen image paths (used as reference)
    """
    product_images = product_images or []
    tv_screens = tv_screens or []
    ...
    if progress_callback:
        progress_callback(10, "Building storyboard...")
    ...
    if generate_images:
        if progress_callback:
            progress_callback(20, "Starting image generation...")
        
        # Use product-based generation if product images provided
        if product_images:
            generated = generate_keyframes_with_products(
        ...
        if progress_callback:
            progress_callback(100, "Storyboard generated (no images).")
