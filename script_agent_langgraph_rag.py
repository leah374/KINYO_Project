...
PROCESSED_KB_DIR = BASE_DIR / "processed_knowledge_base"
EVALUATION_STANDARD_PATH = BASE_DIR / "previous_knowledge_base" / "内容评价标准库_v4.json"
LOCAL_VECTOR_DIR = BASE_DIR / "vector_db" / "local_processed_v1"
FAISS_VECTOR_DIR = BASE_DIR / "vector_db" / "faiss_processed_v1"
...
def load_evaluation_standard() -> Dict[str, Any]:
    ...
    candidates = [
        ...
        BASE_DIR / "内容评价标准库_v4.json",
        BASE_DIR / "previous_knowledge_base" / "内容评价标准库_v4.json",
        BASE_DIR / "knowledge_base" / "内容评价标准库_v4.json",
    ]
    for path in candidates:
        if path.exists():
            return json.loads(path.read_text(encoding="utf-8"))
    searched = "\n".join(str(path) for path in candidates)
    raise FileNotFoundError(f"找不到内容评价标准库_v4.json，已检查：\n{searched}")
...
def build_processed_vector_documents() -> Dict[str, List[Dict[str, Any]]]:
    ...
    for case in structured_cases:
        ...
        case_doc = "\n".join(
            [
                ...
                f"script_pattern_free: {case.get('script_pattern_free', '')}",
                f"hook_type_free: {_json_text(case.get('hook_type_free', []), 500)}",
                f"story_type_free: {_json_text(case.get('story_type_free', []), 500)}",
                f"persona_free: {_json_text(case.get('persona_free', []), 500)}",
                f"scene_free: {_json_text(case.get('scene_free', []), 500)}",
                f"product_display_free: {_json_text(case.get('product_display_free', []), 700)}",
                f"pain_points_free: {_json_text(case.get('pain_points_free', []), 700)}",
                f"selling_points_free: {_json_text(case.get('selling_points_free', []), 700)}",
                f"proof_methods_free: {_json_text(case.get('proof_methods_free', []), 700)}",
                f"cta_type_free: {_json_text(case.get('cta_type_free', []), 500)}",
    ...
    if inventory_path.exists():
        ...
        for key, values in inventory.items():
            ...
            rules.append(
                {
                    "id": f"processed_tag_inventory_{key}",
                    "document": f"free_tag_dimension: {key}\ntop_tags: {_json_text(values[:20], 2600)}",
                    "metadata": {"target": "all", "rule_type": "free_tag_inventory", "dimension": str(key)},
                }
            )
...
def run_agent(brief: str, objective: str, duration: int) -> Dict[str, Any]:
    """运行完整脚本生成 Agent：创建图工作流并传入 brief、目标和时长参数。"""
    app = build_graph()
    return app.invoke(
        {
            "user_input": brief,
...
def main():
    ...
    parser.add_argument("--out", default=str(OUTPUT_DIR / "langgraph_real_rag_result.json"))
